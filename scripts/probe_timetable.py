...
def _scan_service_uris(obj, out: list[str]) -> None:
    """Recursively collect service URIs from any payload shape (km4city or servicemap host)."""
...
async def _florence_stop(client: Client, agencies):
    """Reliable Florence stop via the proven line-6 chain (generic tpl_lines extraction is
    flaky on 888-48's 174 lines). Returns (agency_uri, stop_uri, stop_name)."""
    agency_uri = _resolve_agency(agencies, "")
    if not agency_uri:
        return None, None, None
    routes = await _call(client, "tpl_routes_by_line", {"line": "6", "agency": agency_uri})
    route_uris = _route_uris(routes)
    if not route_uris:
        return agency_uri, None, None
    stops = _stop_entries(await _call(client, "tpl_stops_by_route", {"route": route_uris[0]}))
...
async def main() -> None:
    ...
    async with Client(cfg) as client:
        ...
        flo_uri, flo_stop, flo_name = await _florence_stop(client, agencies)
        print(f"- Florence urban (line 6): stop={flo_name!r}", flush=True)
        if flo_stop:
            print(f"    {_timetable_signature(await _call(client, 'tpl_stop_timeline', {'stop': flo_stop}))}",
                  flush=True)
        for atext in ("ATAC", "Athens"):
            agency_uri, stop_uri, stop_name = await _resolve_stop(client, agencies, atext)
            if not stop_uri:
                print(f"- {atext}: could not resolve a stop (agency_uri={agency_uri})", flush=True)
                continue
            tl = await _call(client, "tpl_stop_timeline", {"stop": stop_uri})
            print(f"- {atext}: stop={stop_name!r}\n    {_timetable_signature(tl)}", flush=True)
        ...
        # Find sensor-ish categories (these carry realtime in km4city), then search for them.
        cats = await _call(client, "get_service_categories", {"mode": "detailed"})
        cat_names = [c.get("subcategory") for c in _generic_list(cats)
                     if isinstance(c, dict) and isinstance(c.get("subcategory"), str)]
        sensor_cats = [c for c in cat_names if any(
            k in c.lower() for k in ("sensor", "weather", "traffic", "parking", "air", "meteo",
                                     "pollution", "bike_sharing", "car_sharing", "charg"))]
        print(f"sensor-ish categories: {sensor_cats[:12]}", flush=True)
        cat_arg = ";".join(sensor_cats[:8]) if sensor_cats else None
        near = await _call(
            client, "service_search_near_gps_position",
            {"latitude": FLO_LAT, "longitude": FLO_LON, "maxdistance": 2.0,
             **({"categories": cat_arg} if cat_arg else {})},
        )
        print("service_search head:", _full(near)[:400], flush=True)
        uris: list[str] = []
