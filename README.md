...
| A `public_transport` request takes ~30–45 s | Known and accepted, not a client bug: the online whatif-router rebuilds the PT graph on every PT request (`docs/lessons.md` L42/L46). `BUS_ROUTE_TIMEOUT_S=120` covers it. Foot/car never touch the PT graph (~0.3–0.5 s). |
