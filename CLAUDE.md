...
- **whatif-router URL 可覆盖**: `bus_route` (PT 公交线) 默认打线上 `https://www.snap4city.org/whatif-router/route`; env `S4C_WHATIF_ROUTER_URL` 可指向本地灌了 Toscana GTFS 的 whatif-router 容器 (见 `whatif-local/`, 经 cloudflared 隧道让 JupyterHub 侧 server 够到本地 :8080)。referente 把数据放线上后清掉 env 回默认。
