...
## 1. Project Overview
**Langgraph MCP client** for referente's remote Snap4City mobility advisor server (UNIFI Sistemi Distribuiti elaborato Tipo A). 真实 MCP server 归 referente 部署在内网 (Snap4City JupyterHub 内网直连访问), 本项目只交付 **client + Langgraph orchestrator + FastAPI 桥 (`api.py`) + dashboard 前端聊天框 (`frontend/`) 测试胶水**。User asks a trip/transport question → Langgraph **deterministic graph** (understand → execute → respond) drives the flow: Llama4 只在 `understand` (forced slot 提取) 和 `respond` (措辞) 出场, **从不自由调工具**; `execute` 用 Python 确定性跑 MCP 工具流 (route: geocode×2 + routing(mode); tpl_*: `tpl.py` 的 `run_tpl_flow` 发现链) → returns widget JSON for the Snap4City dashboard to render. 见 lesson L13 (为何砍掉 `agent ⇄ tools` agentic 回路)。支持 point-to-point **route** (foot/car/public_transport) 和 **tpl_* 公交发现** (tpl_lines/routes/stops/timeline, 见 `tpl.py`); 只有 `other` intent 返友好 unsupported。已知服务端限制 (客户端无误, 已报 referente): stop timetable/realtime 空 (L21)。**两个本地工具兜底 referente 缺口** (都在本地 MCP server `mcp_server.py`, client 经独立 single-server client 连, 避 L6 前缀): (1) **forward geocode** — referente 的 `address_search_location` 服务端坏 (L28), 本地 `address_search_location` 包公开 km4city ServiceMap (L29); (2) **public_transport 路由** — referente 的 `routing` 的 `public_transport` 永不返真公交 (任何日期/OD 都退化纯步行或 -2, L19), 本地 `bus_route` 包 Snap4City What-If GraphHopper router (`vehicle=bus`, Gea-Night dashboard 同源, L19)。`execute` 的 PT mode 改调本地 `bus_route` 出真公交线; foot/car routing、reverse、tpl_* 仍走 referente 远程。即本项目现交付 client + Langgraph + 桥 + 前端 + **一个本地 MCP server (geocode + bus_route)**。
...
- **whatif-router URL (默认已改本地)**: `bus_route` (PT 公交线) **默认打本地 `http://localhost:8080/whatif-router/route`** (`mcp_server.py` 的 `WHATIF_ROUTER_URL` 写死, 因线上实例无 GTFS 只返步行退化 L31/L34; 本地跑灌了 Toscana GTFS 的 whatif-router, 见 `whatif-local/`, 与 mcp_server 同在 JupyterHub 走 localhost)。env `S4C_WHATIF_ROUTER_URL` 仍可覆盖。**referente 把 GTFS + perf patch 放线上后**: 把该默认改回 `https://www.snap4city.org/whatif-router/route` (或设 env 指线上), 本地 Tomcat 即不再需要。
