...
**桥的协议 = job + poll** (L47, 2026-07-13): `POST /advise` 只**启动**这一轮并秒回 `{job_id}`, 前端每 1.5s `GET /advise/{job_id}` (202 = 还在算 + **`stage`/`elapsed_s`** 让前端 thinking 气泡说出"在干什么" (understand/geocode/routing/routing_bus/respond, 由 `run_advisor(on_stage=...)` 回调上报), 200 = widget JSON 原样透传; job_id 和 stage 都只活在传输层不进 payload, 不违规则 8)。原因: bus 轮次 ~50-70s, 反代链把超 60s 的**单个请求**掐断 (后端其实已成功), 而喂心跳字节**无效** —— jupyter-server-proxy 对非 `Accept: text/event-stream` 的响应整体 buffer, 一个字节都不外流。**禁把它改回一次请求拿结果** (线上换反代后赌注只会更差)。
**默认三模式** (L31, 2026-07-13 改): 用户**不指定** mode → foot + car + public_transport **三条全跑全画** (一把扁平 `asyncio.gather` 并发, 墙钟 = 最慢的 bus 那条 30-45s, 因线上 router 未合 perf patch 每 `vehicle=bus` 请求重建 PT 图; patch 合入后归零); **指定 mode → 只跑那一条** (问 "a piedi" 不该等 bus)。整轮**一次答完** (等 bus 一起出结果): 曾试过"先答 foot/car、bus 算完再续写"的两相渐进方案, **已整体回退** —— 简单项目不值得为它背一套两阶段状态机 (前端要管半截气泡、后端要管两次 respond 的措辞边界, Llama4 还不可靠遵守"只写公交那段"), 等待期间有 stage/秒数进度条即可。**出发时刻**: understand 抽 `departure_time` slot ("alle 18" → `18:00`; 今天日期注入 prompt 让它能给 `YYYY-MM-DDTHH:MM`), 只喂给 public_transport 的 `startdatetime` (GraphHopper 无时变 foot/car 模型, 传了等于假装有精度); 到达时刻 ("arrivare per le 9") **不支持** —— What-If servlet 无 `arrive_by` 参数。**geocode 有进程级 LRU 缓存** (km4city 地址索引是静态的; 测试须经 conftest 的 autouse fixture 清空, 否则缓存命中会跳过 FakeClient 队列里的响应, 后续每个 pop 全部错位)。
