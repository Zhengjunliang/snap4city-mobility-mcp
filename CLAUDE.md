...
## 1. Project Overview
**Langgraph MCP client** for referente's remote Snap4City mobility advisor server (UNIFI Sistemi Distribuiti elaborato Tipo A). 真实 MCP server 归 referente 部署在内网 (Snap4City JupyterHub 内网直连访问), 本项目只交付 **client + Langgraph orchestrator + FastAPI 桥 (`api.py`) + dashboard 前端聊天框 (`frontend/`) 测试胶水**。User asks a trip question → Langgraph **deterministic graph** (understand → execute → respond) drives the flow: Llama4 只在 `understand` (forced slot 提取) 和 `respond` (措辞) 出场, **从不自由调工具**; `execute` 用 Python 确定性跑 MCP 工具流 → returns widget JSON for the Snap4City dashboard to render. 见 lesson L13 (为何砍掉 `agent ⇄ tools` agentic 回路)。支持 point-to-point **route** (foot/car/public_transport), **GPS "near" 端点解析** (L41): 前端每轮随 `/advise` 发浏览器 `gps: {lat,lng}|null` → 专名 geocode **全球无区域锁** (用户点名的城市优先 `_narrow_by_city`; 无城市时 `_pick_coord` 按 haversine 取离锚点最近的候选 — 终点锚定已解析起点、起点锚定 GPS, 见 L43; 注意: 代码不设区域限制, 但 km4city **数据实际仅 Toscana 有效** — 无 Brescia/Milano 街道, 库外查询只返模糊噪音, 测试须用 Toscana 内地点 (150km GPS 哨兵已删 2026-07-10: 用户点名异地城市是合法查询, 见 L41); origin 缺省 = GPS 坐标本身 (调一次 `coordinates_to_address` 给回复标签, 无 GPS 则 missing_place 追问); 通用类别目的地 ("farmacia più vicina", understand 的 `destination_category` slot 出英文 km4city 类别) 走 referente `service_search_near_gps_position` 半径阶梯 0.5→2→10 km 取最近, 全空退文本 geocode。只有 `other` intent 返友好 unsupported (公交线路/时刻表 reference 问题已随 tpl_* 删除, 不再支持)。**两个本地工具** (都在本地 MCP server `mcp_server.py`, client 经独立 single-server client 连, 避 L6 前缀): (1) **forward geocode** — referente 的 `address_search_location` 服务端坏 (L28), 本地 `address_search_location` 包公开 km4city ServiceMap (L29; 无 selection 偏置, 对排序零作用 L41); (2) **统一 `route` 工具 (foot/car/bus 全部路由)** — 包 Snap4City What-If GraphHopper router (Gea-Night dashboard 同源); referente 的 `routing` 工具已**整体退役** (2026-07-13, L46: 其 public_transport 永不返真公交 L19, km4city 怪癖还要一整套 stale 重试梯 L3/L8; 实测 whatif foot 0.46s/car 0.31s 反而比它快 — PT 图重建只咬 `vehicle=bus`)。仍走 referente 远程的只剩 reverse geocode、near-search、parking 实时 3 个工具。即本项目现交付 client + Langgraph + 桥 + 前端 + **一个本地 MCP server (geocode + route)**。
**桥的协议 = job + poll** (L47, 2026-07-13): `POST /advise` 只**启动**这一轮并秒回 `{job_id}`, 前端每 1.5s `GET /advise/{job_id}` (202 = 还在算 + **`stage`/`elapsed_s`** 让前端 thinking 气泡说出"在干什么" (understand/geocode/routing/routing_bus/respond, 由 `run_advisor(on_stage=...)` 回调上报) + **`partial`** = 已经能给的半截答案 (完整 widget JSON, 由 `run_advisor(on_partial=...)` 上报, 见 L48); 200 = 完整 widget JSON 原样透传; job_id / stage / partial 都只活在传输层不进 payload, 不违规则 8)。原因: bus 轮次 ~50-70s, 反代链把超 60s 的**单个请求**掐断 (后端其实已成功), 而喂心跳字节**无效** —— jupyter-server-proxy 对非 `Accept: text/event-stream` 的响应整体 buffer, 一个字节都不外流。**禁把它改回一次请求拿结果** (线上换反代后赌注只会更差)。
**默认三模式 + 两相渐进回答** (L31/L48, 2026-07-13 改): 用户**不指定** mode → foot + car + public_transport **三条全跑全画**; **指定 mode → 只跑那一条** (问 "a piedi" 不该等 bus)。但 bus 那条要 30-45s (线上 router 未合 perf patch, 每 `vehicle=bus` 重建 PT 图), foot/car 亚秒 → **图拆两相**: `understand → execute` (只 await foot/car + parking, **bus 甩后台 `asyncio.create_task`**) `→ respond_fast` (措辞前半段, 经 `on_partial` 把完整 widget JSON 交给桥 → 下一个 202 带 `partial` 出去 → 前端 ~3s 就出字出线, 输入框仍锁) `→ execute_pt` (await 后台 bus, 真正的等在这) `→ respond` (只写公交段, **拼到前半段后面成为唯一一条 assistant 消息** → 前端同一气泡改写)。显式 bus (无快结果可先答) 走 `execute → execute_pt → respond` 一次答完; 显式 foot/car 或 unsupported 走 `execute → respond`。**Task 和 callback 一律经 `functools.partial` 绑节点, 禁入 `AdvisorState`** (状态须可序列化); **bus task 必须在 `execute` 最末创建** (parking enrich 之后), 否则打碎 L33 的 FakeClient FIFO 调用序。**出发时刻**: understand 抽 `departure_time` slot ("alle 18" → `18:00`; 今天日期注入 prompt 让它能给 `YYYY-MM-DDTHH:MM`), 只喂给 public_transport 的 `startdatetime` (GraphHopper 无时变 foot/car 模型, 传了等于假装有精度); 到达时刻 ("arrivare per le 9") **不支持** —— What-If servlet 无 `arrive_by` 参数。**geocode 有进程级 LRU 缓存** (km4city 地址索引是静态的; 测试须经 conftest 的 autouse fixture 清空, 否则缓存命中会跳过 FakeClient 队列里的响应, 后续每个 pop 全部错位)。
...
- **Frontend**: `frontend/mobility_advisor_dashboard.html` (CSBL HTML+JS 贴进 Snap4City widgetExternalContent, **三种 mode 全走 widgetMap manual 分支**直画后端几何 — bus 用后端切好的 per-leg 几何 (`routes[].legs`, 双色 + Gea-Night 公交车 icon pin), foot/car 整条 wkt 单 leg; widget 零 router 外呼, 图与字同到, 见 L44/L46; 规则 9 + `frontend/README.md`)
...
- **whatif-router URL (默认线上, 2026-07-10)**: `route` 工具 (foot/car/bus 全部) **默认打线上 `https://www.snap4city.org/whatif-router/route`** — referente 已灌 Toscana GTFS (at+gest), 实测返真公交, 本地自托管不再必需 (运行 harness 脚本已删)。`whatif-local/` 现只留 `patches/` (给 referente 的 perf patch) + 应用/测试说明; 要测本地补丁构建才设 env `S4C_WHATIF_ROUTER_URL` 覆盖成本地 router。**注意**: 线上还没合 `pt-router-singleton` perf patch → 每 `vehicle=bus` 请求重建 PT 图 ~30-45s (`BUS_ROUTE_TIMEOUT_S=120` 兜住); **foot/car 不碰 PT 图, 实测 0.3-0.5s** (L46), 走通用超时; patch 合入后可把 bus 超时收回通用值。
