...
- **whatif-router URL (默认已回线上, 2026-07-10)**: `bus_route` (PT 公交线) **默认打线上 `https://www.snap4city.org/whatif-router/route`** — referente 已灌 Toscana GTFS (at+gest), 实测返真公交; 本地 Tomcat harness (`whatif-local/`) 不再必需, 只留作开发测试 (env `S4C_WHATIF_ROUTER_URL` 指回 `http://localhost:8080/whatif-router/route` 即用)。**注意**: 线上还没合 `pt-router-singleton` perf patch → 每 PT 请求 ~30-40s (`BUS_ROUTE_TIMEOUT_S=120` 兜住); patch 合入后可把该超时收回通用值。
