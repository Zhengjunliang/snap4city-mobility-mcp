...
## 1. Project Overview
**Langgraph MCP client** for referente's remote Snap4City mobility advisor server (UNIFI Sistemi Distribuiti elaborato Tipo A). 真实 advisor MCP server 归 referente (内网, JupyterHub 直连)。本项目交付: **client + Langgraph orchestrator + FastAPI 桥 (`api.py`) + dashboard 前端聊天框 (`frontend/`) + mobility MCP server (`mcp_server.py`: geocode + route, 现托管在 Snap4City server)**。

- **确定性图** (L13): `understand → execute → respond`。Llama4 **只在 `understand`** (forced tool call 抽 slot) 出场, **从不自由调工具**; `execute` 纯 Python 跑工具流; `respond` 纯 Python 确定性出意语措辞 (`_compose_reply`, 无 LLM — L57 移除, respond 输出全部可由 execute 数据推导) → 出 widget JSON 给 dashboard 画。
- **支持范围**: point-to-point route (foot/car/public_transport) + **沿途服务** (用户点名类别 "con le farmacie lungo il percorso" → understand 的 `services_category` slot, execute 沿 route 几何采样锚点并发调远程 near-search, foot/car 沿线、bus 只上下车站+步行段, 按 mode 挂 `routes[i].services`, L53)。**纯附近服务搜索** (pins-only, 无 route): 用户 "mostrami le farmacie qui intorno" → understand 的 `nearby` request_type (复用 `destination_category` 载类别 + `origin_text` 载中心地址/città, **不新增 slot**; `_request_to_intent` 映射到内部 intent `nearby`), execute 单近搜 `maxresults=50` (`NEARBY_RADII_KM` 空 rung 扩) 出**顶层 `data.services`** pin 列表 (`{name,lat,lng,uri,distance_km}`, 区别于 route 的**嵌套** `routes[i].services`), 前端 `Array.isArray(data.services)` 判别后**只撒 pin, 不画线不居中** (widget 无 center 事件, 地图停原处 — 已接受); 中心 = 点名地址/città (geocode) 或 GPS, 缺类别→"che tipo?"/缺地点→"dove?" 追问 (追问轮不带 `data.services` 免清图), 回复单/复数计数 (L58)。只有 `other` intent 返友好 unsupported (公交线路/时刻表 reference 问题随 tpl_* 删除, 不再支持); 到达时刻 ("arrivare per le 9") **不支持** (What-If servlet 无 `arrive_by`)。
...
- **工具分布**: mobility MCP server 两个工具 (托管在 Snap4City server; client 用**独立 single-server client** 连, 避 L6 前缀税) — (1) `address_search_location` 包公开 km4city ServiceMap (native 那个同名工具服务端坏, L28/L29); (2) 统一 `route` 工具包 What-If GraphHopper router, **foot/car/bus 全部走它** (Snap4City native 的 `routing` 已整体退役, L46)。bus 乘车段几何在 `route` 内部升级为**真 GTFS shape** (`gtfs_shapes.py` 调公开 km4city tpl API 按线路号+几何评分匹配方向变体、按上下车站切段, L51; 匹配不上退回 router 的站间直线 L44)。远程只剩 3 个: reverse geocode、near-search (parking 近搜带半径阶梯 0.5→1 km, 空 rung 扩搜, L56)、parking 实时。
...
- **Transport**: HTTP Streamable → Snap4City server 上两个 MCP 端点 (native advisor + mobility; JupyterHub 内网直连 `192.168.1.117:8000`, 见 §5)
...
- **External**: Snap4City server 上两个 MCP server (native advisor + 我们的 mobility, 内网 / JupyterHub 直连), Snap4City Agentic LLM (Langgraph integration)
...
| mobility MCP server (geocode + route, 见 L29/L46) | 已托管在 Snap4City server (`$S4C_DASHBOARD_URL/snap4agentic-mobility-advisor/mcp`); client 默认经 `S4C_LOCAL_MCP_URL` 连, **无需手动起**。本地自测: `python -m snap4city_mobility_mcp.mcp_server` (:8020, 工具 `address_search_location` + `route`) + `export S4C_LOCAL_MCP_URL=http://127.0.0.1:8020/mcp` |
...
- **JupyterHub 上只跑桥一个进程** (uvicorn); mobility server 托管在 Snap4City server — 改了 `mcp_server.py` 要 Snap4City 那边重部署才生效。**本地自测**时 :8020 与桥两进程都要重启 (L54: 漏重启曾伪装成 civic 全 None 端点漂移)。
...
| 项 | 值 |
|---|---|
| MCP server | Snap4City server `192.168.1.117:8000` 上两个: native advisor (via apps.json) + mobility (`/snap4agentic-mobility-advisor/mcp`); orchestrator 默认已指这 |
