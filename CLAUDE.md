...
**桥的协议 = job + poll** (L47, 2026-07-13): `POST /advise` 只**启动**这一轮并秒回 `{job_id}`, 前端每 1.5s `GET /advise/{job_id}` (202 = 还在算, 200 = widget JSON 原样透传, job_id 只活在传输层不进 payload)。原因: bus 轮次 ~50-70s, 反代链把超 60s 的**单个请求**掐断 (后端其实已成功), 而喂心跳字节**无效** —— jupyter-server-proxy 对非 `Accept: text/event-stream` 的响应整体 buffer, 一个字节都不外流。**禁把它改回一次请求拿结果** (线上换反代后赌注只会更差)。
