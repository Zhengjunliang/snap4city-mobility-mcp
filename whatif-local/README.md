...
2. **每请求全量重载图, 极慢** — 原版 whatif-router 每个 PT 请求都 `importOrLoad()` 反序列化 ~2GB
   ...
   仍 ~30-40s (`mcp_server` 用 `BUS_ROUTE_TIMEOUT_S=120` 兜住)。

`mcp_server.py` 默认已指线上, 本地/JupyterHub 自托管不再必需 — 本目录只留作开发 harness
(测 patch 后构建 / 换别的 GTFS 时用, `S4C_WHATIF_ROUTER_URL` 指回 localhost)。
...
测完清 `S4C_WHATIF_ROUTER_URL` 回默认 (线上实例), 停 Tomcat (`stop` 子命令)。
