...
**Backend reference** — field-by-field notes from live-probing the km4city endpoints behind referente's remote MCP server. §1 / §2 cover the underlying km4city `/location/` + `/shortestpath` semantics (baseline); the **actual remote tool signatures** the advisor calls (bare names — a single-server config adds no prefix, see lesson L6) live in §3.
...
### Binding gotchas (see lessons L11 / L17 / L20)

- **Not region-locked**: the index now holds Valencia (ES) / southern France / Maastricht (NL) entries — `"...Firenze"` can return 100 out-of-region hits and zero Tuscan. There is **no geo-constraint parameter**; the advisor pins results to a Tuscany bbox client-side ([mcp_tools._filter_geocode_to_tuscany](../src/snap4city_mobility_mcp/mcp_tools.py)).
...
- **Same-name towns**: `"Piazza Duomo"` also matches squares in Castelnuovo / Pietrasanta (90 km away). A city-name ladder (named city → Florence default → all Tuscany) narrows it ([mcp_tools._narrow_by_city](../src/snap4city_mobility_mcp/mcp_tools.py)).
- **Pure-noise input → HTTP 500**, not an empty FeatureCollection. Callers must tolerate 5xx, and an empty / `[]` result is NOT a clean "no match" signal.
- **Backend is non-deterministic over time** (L20): the same string can return all-foreign one minute and the correct Tuscan hit the next. A bounded retry fires only when the bbox filter leaves zero Tuscan hits.
...
### Critical: `error_code == "0"` is the ONLY success signal (L7); `ok` / HTTP 200 ≠ route found

`response.error_message` is non-empty even on success (`"successful"`) — never use it as a failure signal. `journey.routes` may be `[]` even on a 200 (car in a pedestrian zone, src==dst). Higher-level callers **must** check `len(routes) > 0`. `error_code != "0"` → failure (`-2` = route not found, `-1` = wrapper internal error). `arc[i].desc` gives turn-by-turn text; `"nd"` entries are common (label as "unnamed street").

The advisor's `respond` surfaces `routes[0].{wkt, distance, eta, time}` + source/destination_node into the widget `data` (full WKT) via [orchestrator._extract_data](../src/snap4city_mobility_mcp/orchestrator.py). `arc` per-segment detail is currently commented out to slim the payload — re-enable if the dashboard widget needs it.
...
Names appear **without server prefix** under a single-server config (as we use). FastMCP only prefixes when merging multiple servers. So `address_search_location`, not `snap4agentic_advisor_native_address_search_location` — see lesson L6 and the `project-referente-endpoint` memory.
...
- **Shape A (top-level empty wrap)** — `{"error": ""}`, no `journey`. Causes: L3 short-window stale (transient, clears ≥ 5 s later) **or** L8 / L19 server-side wrapper bug (stable: car-in-ZTL, and in fact **all** car / public_transport requests — retries don't clear).
...
- **Shape C (empty routes, success envelope)** — `error_code = "0"` but `journey.routes = []` (L2: graph search found no path, e.g. car in a pedestrian zone). Surfaced as `"no route found (empty routes list)"`.

### TPL chain — live shapes (2026-06-12, see lesson L21)
...
- `tpl_stops_by_route` → `[service-URI array, {"BusStops": {"features": [...]}}]` — GeoJSON nested under `BusStops` (no top-level `type`); each feature carries stop `name` / `serviceUri` / coordinates under `properties`.
- `tpl_stop_timeline` → `{"BusStop": {features:[...]}, "busLines": {results:{bindings:[...]}}, "realtime": {}, "timetable": {}}` — gives the stop + serving lines, but `realtime` / `timetable` came back **empty** (no datetime param; scheduled times appear unavailable server-side). `respond` reports the serving lines and says times are unavailable, never inventing them.
...
- Will referente fix the car / public_transport empty-body bug? `routing` returns `{"error": ""}` for car (even a drivable non-ZTL destination) and public_transport (even with `startdatetime`), while foot_* work — server-side, see lesson L19.
