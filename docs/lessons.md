...
L14 多轮对话状态**早已存在**于对外输出 JSON 的 `messages` (OpenAI 格式, `run_advisor(query, history)→{...,messages}`) —— 想加聊天前端时别"从头造多轮", 它只是把上轮 `messages` 回传当 `history` 的纯胶水。陷阱: 加新聊天 UI (Chainlit) 时若**留着**旧终端 REPL/CLI, 就成**两份多轮实现** = 平行实现, 违单一正确实现 (CLAUDE.md §2 规则7) → 删掉 `cli.py` + `[project.scripts]` 入口, Chainlit 成唯一聊天入口。两个落地点: (a) `run_advisor` 本就 async → Chainlit async handler **直接 `await`** (旧 CLI one-shot 才需 `asyncio.run`); (b) UI **只显 LLM 原话** `messages[-1].content` (`respond` 已把距离/ETA 措辞进自然语言并兜底), **不在 UI 硬编码** `📍km·ETA`; 全量 widget JSON (含 `data.wkt`) 每轮追加 `outputs.txt` 供离线看流程。JupyterHub 运行 caveat: Chainlit 是 **web server** (非纯终端) → `chainlit run chainlit_app.py --host 0.0.0.0 --port 8501` (避开 MCP dashboard 的 `:8000`), 浏览器开 `<JupyterHub URL>/proxy/8501/` (需 `jupyter-server-proxy`)。Chainlit 拉 fastapi/uvicorn 重依赖 → 单独 `[dependency-groups] ui`, 不进核心 `dependencies`。
