...
l'orchestratore Langgraph, (iii) un bridge FastAPI (\texttt{api.py}) con il
front-end della dashboard (una chat box, \texttt{frontend/}) come collante di
test. Nessuno strumento è implementato lato client.
...
\texttt{response}     & il widget JSON assemblato da \texttt{respond} \\
\bottomrule
\end{tabular}
...
campo proprietario aggiuntivo (Listato~\ref{lst:response}).

\begin{lstlisting}[caption={Forma del widget JSON restituito.}, label={lst:response}]
{
...
% =============================================================================
\section{Interfaccia di test (dashboard + bridge \texttt{api.py})}
% =============================================================================
Il front-end è una chat box in linguaggio naturale sulla dashboard Snap4City (un
\texttt{widgetExternalContent}) che dialoga con un bridge FastAPI
(\texttt{api.py}): \texttt{POST /advise} incapsula \texttt{run\_advisor} e
restituisce verbatim il JSON del widget. Consente il test multi-turno: si digita
una domanda, si ottiene la risposta, si prosegue (le domande di follow-up come
``\textit{e in bus?}'' sono risolte rispetto alla storia, rispedita come
\texttt{history} a ogni turno). Nella bolla di chat vengono mostrate solo le
parole dell'LLM (\texttt{messages[-1].content}), mentre il percorso
(\texttt{data.wkt}) è disegnato su un \texttt{widgetMap} affiancato; l'intero
JSON del widget viene accodato a \texttt{outputs.txt} per l'ispezione offline e
i log di diagnostica vanno in \texttt{debug.log}.

\paragraph{Motivazione progettuale.} Inizialmente, dato che l'unico ambiente di
esecuzione è JupyterHub --- dove esporre una porta web richiedeva un proxy
fragile e il riavvio del server, con il rischio concreto di rendere
inutilizzabile l'intero singleuser server --- si era usata una REPL da terminale
come forma di transizione. Individuata una procedura sicura per installare
\texttt{jupyter-server-proxy} (senza aggiornare i pacchetti Jupyter di base), la
via web è diventata percorribile: il browser raggiunge il bridge \emph{stessa
origine} attraverso il proxy, senza preflight CORS. Si è quindi adottata la chat
box sulla dashboard come interfaccia finale e, per il principio di unica
implementazione, la REPL di transizione è stata rimossa.
