...
**每条都标 ✅ (能跑通) 或 ⚠️ (已知服务端限制, 诚实降级) — 全部可复现, 无模糊案例。**
...
>   client 无误 (见 `docs/lessons.md` L19/L21)。Group E 演示这些的**诚实降级** (无编造、无乱报 ZTL)。
...
Duomo 起点、只改终点。**follow-up 轮回复不再以 `Ciao` 开头、不带 "non esitare a chiedere" 收尾客套。**
...
每条**故意**触发已知空/退化, 重点看 client **诚实措辞、不编造数字、不乱说 ZTL**。
...
> **不给任何数字**、请用更精确地址。**不**乱说 ZTL (foot 失败不触发 ZTL hint)。
